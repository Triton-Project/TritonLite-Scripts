 ...
 *
 * @file testcode.ino * 
 * @brief 2024/01/28の実験で使用するテストコード
...
void loop() {
  ...
  acquireSensorData();  // センサーデータの取得と処理
  if (in_prs_pressure * 68.94 < out_pre_pressure) {  //PSIをmbarに変換 閾値の調整頼む
...
void acquireSensorData() {
  ...
  in_prs_pressure = in_prs_voltage / 2.7 * 30;  //PSI
}
...
void writeSDcard() {
/**
 * ここのSDに書き込む際,必要なメモリ容量が大きいためメモリ不足が発生している
 * 必要なデータのみ書き込むことでメモリ削減が可能である
*/
  String Sensorlog = "";
  Sensorlog += miliTime;
